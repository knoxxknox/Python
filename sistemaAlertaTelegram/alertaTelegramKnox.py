...
def marcaEnvioTXT(linha, linhaAlterada, diaSemana):
    arquivo = open('/Users/knox/python/SistemaDeAlerta/' + diaSemana + '.txt','r')
    with arquivo as fd:
        ...
        with open('/Users/knox/python/SistemaDeAlerta/' + diaSemana + '.txt','w') as fd:
    ...
    if msgEnviada == "N" and status == "AT":
        if today.hour >= horaArquivo:
            if today.minute >= minutoArquivo:
                ...
                time.sleep(1)
                bot.sendMessage(idTelegram, ("*<<ALERTA>>*\n" + msg), parse_mode= 'Markdown')
    ...
    if soma == 5:
        ...
        if critica == False:
                if (int(numPrimeiro) <= 2) and (int(numTerceiro) <=5):
                    ...
                    arquivo = open('/Users/knox/python/SistemaDeAlerta/' + week + '.txt','r')
...
def listaMensagem(id):
    ...
    for item in diaSemana:
        arquivo = open('/Users/knox/python/SistemaDeAlerta/' + item + '.txt','r')
...
def desativarAlerta(id, dia, idTXT, fNome):
    achou = False
    idTXT = "id" + idTXT[2:5]
    arquivo = open('/Users/knox/python/SistemaDeAlerta/' + dia + '.txt','r')
    for linha in arquivo:
        linha = linha.rstrip()
    ...
    if achou == True:    
        ...
        arquivo = open('/Users/knox/python/SistemaDeAlerta/DOM.txt','r')
        with arquivo as fd:
            arqTotal = fd.read()  # Ler todo o arquivo
            arqTotal = arqTotal.replace(linha, linhaModif) 
        with open('/Users/knox/python/SistemaDeAlerta/DOM.txt','w') as fd:
...
def menu(id):
    arquivo = open('/Users/knox/python/SistemaDeAlerta/MENU.txt','r')
    with arquivo as fd:
        arqTotal = fd.read()  # Ler todo o arquivo
...
def reload():
    diaSemana = ["SEG", "TER", "QUA", "QUI", "SEX", "SAB", "DOM"]
    for item in diaSemana:
        arquivo1 = open('/Users/knox/python/SistemaDeAlerta/' + item + '.txt','r')
        with arquivo1 as fd:
            txt = fd.read()  # Ler todo o arquivo
            txt = txt.replace("-S-", "-N-") 
        with open('/Users/knox/python/SistemaDeAlerta/' + item + '.txt','w') as fd:
            fd.write(txt)  # Escrever texto modificado
        time.sleep(0.5)   
    ...
    if text[:9] == "DESATIVAR":  
        ...
        for dia in diaSemana:
            if dia == text[10:13] and text[14:16] == "ID" and len(idTXT) == 5:
                ...
                break
        if validado == False:
              bot.sendMessage(id, fNome + ", informações *inválidas!*\nPara desativar um alerta envie uma mensagem no formato do exemplo a seguir:\n\n*desativar-sex-id001*\n\nCaso não saiba o id do alerta que deseja desativar, envie *LISTAR* para verificar todos os seus alertas cadastrados.", parse_mode= 'Markdown')
    else:
